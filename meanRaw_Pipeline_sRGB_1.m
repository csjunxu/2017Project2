...
rawname = S{1};
[status, ~] = system(['dcraw -4 -T -D -v C:\Users\csjunxu\Desktop\TIP2017\RID_Dataset\' D{end-1} '\' D{end} '\' rawname '.ARW']);
...
for i = 1:im_num
    ...
    if i == min(500,im_num)
        ...
        %         imshow(meanRaw500);
        imwrite(meanRaw500, [D{1} '/' D{2} 'mean/meanRaw500_ARW2TIF.tiff']);
        clear meanRaw500;
    end
    ...
    if i == min(500,im_num)
        ...
        %         imshow(meansRGB500);
        imwrite(meansRGB500, [D{1} '/' D{2} 'mean/meansRGB500_ARW2TIF_TIF2RGB.png']);
        clear meansRGB500;
    end
...
% imshow(meanRawAll);
imwrite(meanRawAll, [D{1} '/' D{2} 'mean/meanRawAll_ARW2TIF.tiff']);
clear Raw meanRawAll cmdout;
meansRGBAll = uint8(meansRGBAll./im_num);
% imshow(meansRGBAll);
imwrite(meansRGBAll, [D{1} '/' D{2} 'mean/meansRGBAll_ARW2TIF_TIF2RGB.png']);
