...
% Print file names to command window.
for k = 1 : length(files)
    if isempty(strfind(files(k).name, 'mean')) || strcmp(files(k).name, '.') == 1 || strcmp(files(k).name, '..') == 1
        continue;
    end
    fprintf('Sub folder #%d = %s\n', k, files(k).name);
    sRGB_dir       =  fullfile(Original_image_dir, files(k).name);
    mean_dir       =  fullfile(Original_image_dir, [files(k).name 'mean']);
